...
const props = defineProps({
  bambini: { type: Array, default: () => [] },
  diariEsistenti: { type: Object, default: () => ({}) },
  dataSelezionata: String,
  classi: Array
});

// 1. STATO: Inizializziamo con il primo bambino se c'è
const initialId = props.bambini?.length > 0 ? props.bambini[0].id : null;
const selectedBambinoId = ref(initialId);

// 2. FORM
const form = useForm({
  bambino_id: selectedBambinoId.value,
  giorno: props.dataSelezionata,
...
// 3. LOGICA DI SINCRONIZZAZIONE (Il cuore del sistema)
// Questa funzione viene chiamata sia se cambi bambino, sia se cambi data
const sincronizzaForm = () => {
  const currentId = selectedBambinoId.value;
  
  // Se non c'è nessun bambino selezionato, non fare nulla
  if (!currentId) return;

  // A. Aggiorniamo sempre i metadati (ID e Data corrente)
  form.bambino_id = currentId;
  form.giorno = props.dataSelezionata; // <--- FONDAMENTALE: aggiorna la data nel form
  
  // B. Cerchiamo se esiste un diario nei NUOVI props scaricati
  const diario = props.diariEsistenti ? props.diariEsistenti[currentId] : null;
  
  if (diario) {
    // --- C'È UN DIARIO: POPOLA ---
    form.clearErrors();
    form.pasti = diario.pasti ?? { primo: 'tutto', secondo: 'metà', merenda: 'niente' };
    form.sonno = diario.sonno ?? { da: '', a: '' };
    form.bisogni = diario.bisogni ?? [];
    form.attivita = diario.attivita ?? '';
    form.umore = diario.umore ?? 'sereno';
  } else {
...
// CASO A: L'utente clicca su un altro bambino
watch(selectedBambinoId, () => {
    sincronizzaForm();
});

// CASO B: L'utente cambia DATA (props.diariEsistenti cambia dopo la chiamata al server)
watch(() => props.diariEsistenti, () => {
    // Poiché abbiamo usato preserveState: true, selectedBambinoId è rimasto invariato.
    // Chiamando sincronizzaForm, ricarichiamo i dati per LO STESSO bambino, ma con i nuovi diari.
    //console.log('Si aggiornano i diari esistenti:', props.diariEsistenti);
    sincronizzaForm();
}, { deep: true });
...
const onDateChange = (e) => {  
  router.get('/diario', { giorno: e.target.value }, {
    preserveState: true, // <--- MANTIENE IL BAMBINO SELEZIONATO
    preserveScroll: true, // <--- NON FA SCATTARE LA PAGINA IN ALTO
    only: ['diariEsistenti', 'dataSelezionata'] // <--- SCARICA SOLO I NUOVI DATI
  });
};

// ... resto delle funzioni (salva, addBisogno, ecc) ...
const addBisogno = () => { form.bisogni.push({ ora: '10:00', tipo: 'pipì' }); };

const salvaDiario = () => {
  form
  .transform((data) => ({
        ...data,
        giorno: props.dataSelezionata, // Assicuriamoci che la data sia aggiornata
    }))
  .post('/diario', { 
    preserveScroll: true,
    onSuccess: () => {
